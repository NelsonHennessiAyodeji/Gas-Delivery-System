...
const createOrder = async (req, res) => {
  try {
    const { shippingFee, total, status, size } = req.body;
    const { userId: user } = req.user;
    const productId = Math.floor(Math.random() * 9000) + 1000;
    const paymentMethod = "Theo-Funds";
    let l = size;
    l += "kg";

    console.log(shippingFee, total, status, size, user, paymentMethod);
    
    // TODO Check if the user id is valid first 
