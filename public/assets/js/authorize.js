...
// Get references to the balance display and deposit amount input
    ...
    if (paymentBtn) {
      paymentBtn.addEventListener('click', async () => {
        if (!cardNumber.value || !cardDate.value || !cvc.value || !cardOwner.value) {
        ...
        const currentBalance = Number.parseInt(localStorage.getItem("currentBalance"));
        ...
        if (!isNaN(depositAmount)) {
          ...
          const res = await fetch('http://localhost:3000/users/updateWalletBalance', {
            method: "POST",
            headers: {
...
async function fillOrderTable() {
  const response = await fetch('http://localhost:3000/orders/showAllMyOrders', {
    method: "GET",
    credentials: "include"
  });
  ...
  orders.forEach(order => {
    tableBody.innerHTML += `                                                    
    <tr>
    <td><a class="account-order-id" href="javascript:void(0)">#${order.productId}</a></td>
...
async function fetchData() {
    try {
      const response = await fetch('http://localhost:3000/users/showMe', {
        method: 'GET',
        credentials: 'include'
      });
